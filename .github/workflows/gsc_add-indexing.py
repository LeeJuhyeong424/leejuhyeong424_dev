...
def validate_and_parse_input():
    added_files_str = os.getenv("JSON_FILES", "")
    try:
        print(f"받은 추가된 파일 목록: {added_files_str}")

        # 쉼표로 구분된 문자열을 리스트로 변환
        added_files = [file.strip() for file in added_files_str.split(',') if file.strip()]
...
def update_status_file(date, time, result, tooltip, url):
    ...
    new_record = f"| {date_with_tooltip} | 추가 | {result_with_tooltip} | {url} |\n"
...
if __name__ == "__main__":
    added_files = validate_and_parse_input()
